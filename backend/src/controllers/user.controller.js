    ...
    try{
        ...
        if(usuariobuscado){
            ...
            res.json({
                ...
                createdAt: usuariobuscado.createdAt,
                updatedAt: usuariobuscado.updatedAt,
            });
        }
    }
    catch(error){
...
export const actualizarcorreoelectronico = async (req, res)=>{
    const {emailpast,emailnew}=req.body;
    try{
        const usuariobuscado = await User.findOne({email: emailnew});
        if(usuariobuscado){
            await User.updateOne(
                { email: emailnew },
                { $set: { email: emailpast } }
...
export const profile=async (req,res)=>{
    ...
    return res.json({
        id: busquedausuario.id,
        username:busquedausuario.username,
        email:busquedausuario.email,
        createdAt:busquedausuario.createdAt,
        updatedAt:busquedausuario.updatedAt
    })*/
}
