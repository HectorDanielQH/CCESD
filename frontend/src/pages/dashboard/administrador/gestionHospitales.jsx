...
import { CloudIcon, HomeIcon, MagnifyingGlassIcon, PencilIcon, PhoneIcon, SunIcon, TrashIcon, TvIcon } from "@heroicons/react/24/solid";
import { useEffect, useState } from "react";
import { useForm } from "react-hook-form";
import Cookies from "js-cookie";
import { obtenerHospitalRequest, registrarHospitalRequest, editarHospitalRequest, eliminarHospitalRequest } from "../../../api/hospitales";
import { Map, Marker, ZoomControl } from "pigeon-maps"

export default function GestionHospitales(){
    ...
    const nuevoHospital=()=>{
        handleOpen();
        reset();
        setTipo('nuevoHospital');
        setLatlng([0,0]);
    }

    const TABLE_HEAD = ["Nombre del Hospital", "Nivel", "Ubicación", "Teléfono", "Horarios de Visita","Servicios",""];
    ...
    useEffect(()=>{
        ...
        try {
            const r= await obtenerHospitalRequest(pagina,datosbuscados);
            setHospitales(r.data.docs);
    ...
    const busquedaHospital=async (e)=>{
    ...
    const agregarHospital=handleSubmit(async e => {
        try{
            const re=await registrarHospitalRequest(e);
            const r= await obtenerHospitalRequest(pagina,datosbuscados);
            setHospitales(r.data.docs);
            setFiltro(r.data.docs);
            reset();
    ...
    const editarHospital=(e) =>{
        const filtered = hospitales.filter(hospital => 
            hospital._id.toLowerCase().includes(e)
        );
        setTipo("actualizar")
        setValue("hospital",filtered[0].hospital);
        setValue("nivel",filtered[0].nivel);
        setValue("ubicación",filtered[0].ubicación);
        setValue("telefono",filtered[0].telefono);
        setValue("turnomañana",filtered[0].horariosvisita.turnomañana);
        setValue("turnotarde",filtered[0].horariosvisita.turnotarde);
        setValue("servicios",filtered[0].servicios);
        setIdHospital(filtered[0]._id)
        setLatlng(filtered[0].ubicación);
        handleOpen(true);
    }
    

    const guardarHospitalEditado=handleSubmit(async e => {
        try{
            const res=await editarHospitalRequest({_id:idHospital},e);
            const r= await obtenerHospitalRequest(pagina,datosbuscados);
            setHospitales(r.data.docs);
            setFiltro(r.data.docs);
            setAlerta(true);
    ...
    const eliminarHospital=(async (_id)=>{
        const cookies=Cookies.get();
            if(cookies.token){
                try{
                    const res=await eliminarHospitalRequest({_id:_id},cookies.token);
                    const r= await obtenerHospitalRequest(pagina,datosbuscados);
                    setHospitales(r.data.docs);
                    setFiltro(r.data.docs);
                    setAlerta(true);
    ...
    const botonAdelante=async ()=>{
        const r= await obtenerHospitalRequest(pagina+1,datosbuscados);
        setHospitales(r.data.docs);
        setFiltro(r.data.docs);
        setAdelante(r.data.hasNextPage);
        setAtras(r.data.hasPrevPage);
    ...
    const navDirecto=async(e)=>{
        const r= await obtenerHospitalRequest(e , datosbuscados);
        setHospitales(r.data.docs);
        setFiltro(r.data.docs);
        setAdelante(r.data.hasNextPage);
        setAtras(r.data.hasPrevPage);
                ...
                <div className="text-gray-800 text-4xl flex sm:mt-6 lg:mt-0">
                    <div className="sm:w-full sm:flex sm:flex-col sm:mt-10 md:mb-6 sm:justify-center sm:items-center lg:flex-row lg:justify-between lg:w-10/12 lg:mx-auto">
                        <h1 className="sm:text-2xl lg:text-4xl lg:font-extralight flex justify-center items-center">
                            <HomeIcon className="w-7 mr-3"/>Gestión de Hospitales</h1>
                        <Button onClick={nuevoHospital} className="my-4 bg-green-800">Registrar Nuevo Hospital</Button>
                    </div>
                    <Dialog
                        size="xs"
                        open={open}
                    ...
                    >
                        <Card className="mx-auto sm:w-full md:w-full lg:w-[40rem] h-[60vh] overflow-y-scroll">
                            <form onSubmit={tipo==="nuevoHospital"?agregarHospital:guardarHospitalEditado}>
                                <CardBody className="flex flex-col gap-4">
                                    ...
                                    <Input 
                                        label="Ej.: Hospital Juan de Dios"
                                        type="text"
                                        size="lg"
                                        name="hospital"
                                        {
                                            ...register("hospital",{
                                                ...
                                                pattern:{
                                                    value: /^[A-Z\s]+$/,
                                                    message:"El Hospital solo puede contener MAYÚSCULAS"
                                                }
                                        })}
                                    />
                                    {errors.hospital && <Alert className="text-red-800 rounded-lg bg-red-100 py-1">{errors.hospital.message}</Alert>}

                                    <div className="flex w-full flex-col gap-10">
                                        <Typography className="-mb-2" variant="h6">
                                            Nivel:
                                        </Typography>
                                        <div className="w-full flex sm:flex-col lg:flex-row lg:gap-7 justify-center items-center -mt-8">
                                            <Radio 
                                                name="nivel" 
                                                label="Primer Nivel"
                                                value={"Primer Nivel"}
                                                {...register("nivel", { 
                                                    required:{
                                                        value:true,
                                                        message: "El nivel de hospital es requerido" 
                                                    }
                                                })} 
                                            />
                                            <Radio 
                                        ...
                                        {errors.nivel && <Alert className="text-red-800 rounded-lg bg-red-100 py-1">{errors.nivel.message}</Alert>}
                                    </div>
                                    ...
                                    <Textarea
                                        color="gray" 
                                        label="Ej.: Los servicios que el hospital tiene son: Especialidad en...."
                                        name="servicios" 
                                        {
                                            ...register("servicios",{
                                                required:{
                ...
                <div className="w-11/12 mx-auto">
                    <Card className="h-full w-full">
                        ...
                        <CardBody className="overflow-x-scroll px-0 sm:w-10/12 mx-auto">
                            ...
                            <tbody>
                                {filtro.map(({_id,hospital,nivel,ubicación,telefono,horariosvisita,servicios}, index) => {
                                    const isLast = index === hospitales.length - 1;
                                    ...
                                    <tr key={_id}>
                                        ...
                                        <td className={classes}>
                                            <Typography
                                                variant="small"
                                                color="blue-gray"
                                                className="w-64 font-normal truncate overflow-hidden whitespace-nowrap"
                                            >
                                                <Map 
                                                    height={300}
                                                    center={ubicación} 
                                        ...
                                        <td className={classes}>
                                            <Chip 
                                                className="mb-2" 
                                                color="amber"
                                                value={`Mañana: ${horariosvisita.turnomañana}`}
                                                icon={<SunIcon/>}
                                            />
                                            <Chip 
                                                color="red" 
                                                value={`Tarde: ${horariosvisita.turnotarde}`} 
                                                icon={<CloudIcon/>}
                                            />
                                        </td>
                                        ...
                                        <td className={classes}>
                                            <div className="flex justify-center items-center gap-2">
                                                <Tooltip content="Editar Hospital">
                                                    <IconButton variant="text" className="bg-orange-400 hover:bg-orange-300" onClick={()=> editarHospital(_id)}>
                                                        <PencilIcon className="h-4 w-4" />
                                                    </IconButton>
                                                </Tooltip>
                                                <Tooltip content="Eliminar Hospital">
                                                    <IconButton variant="text" className="bg-red-700 hover:bg-red-400" onClick={()=>eliminarHospital(_id)}>
