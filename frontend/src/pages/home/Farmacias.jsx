...
import { obtenerFarmaciaPageRequest } from "../../api/farmacias";
...
export default function Farmacias(){
    ...
    useEffect(()=>{
        async function farmaciasGet() {
            const r=await obtenerFarmaciaPageRequest();
            setFarmacias(r.data);
            setFiltro(r.data);
        }
        farmaciasGet();
    ...
    const filtrar=(e)=>{
        ...
        const farmaciasFiltradas = farmacias.filter(farmacia => {
          return farmacia.farmacia.toUpperCase().includes(filtro);
        });
        setFiltro(farmaciasFiltradas);
    }
    ...
    <div>
        ...
        <div className="flex flex-col justify-center items-center">
            <div className="mt-20 lg:mt-28 mb-10">
                <Typography variant="h1" className="sm:text-2xl md:text-3xl lg:text-4xl sm:text-center sm:mt-4 md:mt-10 text-blue-gray-500">LISTA DE FARMACIAS EN LA CIUDAD DE POTOSÍ</Typography>
            </div>
            <div className="w-9/12 sm:flex-col lg:flex-row flex justify-center items-center gap-2 sm:mb-6 md:mb-5">
                <h2 className="text-blue-gray-700 text-right lg:mr-5">
                    <strong>
                        Realiza la busqueda por farmacia
                    </strong>
                </h2>
                <Input
                    type="search"
                    label="Busqueda de Hospitales"
                    onKeyUp={filtrar}
                />
            </div>
                ...
                {filtro.map(({_id,farmacia,ubicación,telefono,horariosatencion}, index)=>(
                        <Card className="mt-6 bg-gray-200 pt-11 sm:w-11/12 lg:w-5/12 w-5/12 shadow-2xl" key={_id}>
                            <CardHeader color="white" className="relative h-full">
                                <Typography variant="h4" color="blue-gray" className="bg-white p-4 flex items-center justify-center">
                                    {farmacia}
                                </Typography>
                                <Typography
                                    variant="small"
                            ...
                            <CardBody>
                                ...
                                <Accordion open={open === index+1}>
                                    ...
                                    <AccordionBody>
                                        <Chip
                                            color="amber"
                                            value={"TURNO MAÑANA: "+horariosatencion.turnomañana}
                                        />
                                        <br />
                                        <Chip
                                            color="teal"
                                            value={"TURNO TARDE: "+horariosatencion.turnotarde}
                                        />
                                        <br />
                                        <Chip
                                            color="pink"
                                            value={"TURNO NOCHE: "+horariosatencion.turnonoche}
                                        />
                                    </AccordionBody>
                                </Accordion>                        
                            </CardBody>
